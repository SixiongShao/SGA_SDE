...
class SGA:  
    ...
    def collect_and_rank_terms(self):
        term_mse_dict = {}
        term_object_dict = {}  # Dictionary to store the actual Tree objects

        for i in range(min(20, len(self.eqs))):  # Collect terms from the first 20 PDEs
            pde = self.eqs[i]
            mse = self.mses[i]
            seen_terms = set()
            
            elements = copy.deepcopy(pde.elements)
            elements, coefficients = evaluate_mse(elements, True)
            coefficients = coefficients[1:, 0]

            for j in range(len(coefficients)):
                if np.abs(coefficients[j]) < 1e-4:  # Ignore terms with small coefficients
                    continue

                term = elements[j]
                inorder_term = term.inorder
                if inorder_term not in seen_terms:
                    seen_terms.add(inorder_term)
                    if inorder_term in term_mse_dict:
                        term_mse_dict[inorder_term].append(mse)
                    else:
                        term_mse_dict[inorder_term] = [mse]
                        term_object_dict[inorder_term] = term  # Store the Tree object

        # Calculate average MSE for each term
        term_avg_mse = {term: np.mean(mse_list) for term, mse_list in term_mse_dict.items()}

        # Rank terms by average MSE
        ranked_terms = sorted(term_avg_mse.items(), key=lambda item: item[1])

        # Calculate probabilities
        avg_mses = np.array([item[1] for item in ranked_terms])
        exp_neg_avg_mses = np.exp(-avg_mses)
        sum_exp_neg_avg_mses = np.sum(exp_neg_avg_mses)
        prob = exp_neg_avg_mses / sum_exp_neg_avg_mses

        print("Ranked terms (by average MSE) after every 10 generations:")
        for (inorder_term, avg_mse), p in zip(ranked_terms, prob):
            term = term_object_dict[inorder_term]
            print(f"Term: {term.inorder}, Average MSE: {avg_mse}, Probability: {p}")

        return [term_object_dict[inorder_term] for inorder_term, _ in ranked_terms], prob
